...
const printGrid = () => {
  ...
  for (let i = 0; i < grid.length; i++) {
    console.log(grid[i].slice(startX, 550).join(' '));
  }
  console.log();
}
...
const dropGrain = () => {
  ...
  if (grid[y][x] === 'o') return false; // Reached the end

  if (y >= maxY || x >= maxX) {
    console.error('grid is too small', y, x);
  }
...
// Init grid
const grid = Array(maxY + 3).fill('.').map(() => Array(1000).fill('.'));
...
// Try just adding the line at the bottom of the grid
